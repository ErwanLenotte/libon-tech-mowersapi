...
import com.libon.mowersapi.domain.instruction.MouvementEnum;
...
public class MowersUpdateUseCase {
    ...
    private static final Map<String,Instruction> MAP_STRING_TO_INSTRUCTION
            = Map.of("A",MouvementEnum.A, "D",DirectionEnum.D,"G",DirectionEnum.G);

    public MowersUpdatedDto updateMowers(MowersDetailsDto mowersDetailsDto) {

        MowersDetailsDto details = Optional.ofNullable(mowersDetailsDto)
                .orElse(new MowersDetailsDto(new FieldDto(0, 0), List.of()));
    ...
    private Mower moveMovers(FieldDto field, MowerDto mowerDto) {
        ...
        List<Instruction> instructions = mowerDto.instructions().stream().map(MAP_STRING_TO_INSTRUCTION::get).toList();
